...
mainLayout.btn.addEventListener('click', () => {
  mainLayout.text.classList.remove('pink');
  mainLayout.text.classList.remove('purple');

  if(control < 4){
    if(control === 2){
      mainLayout.text.classList.remove('pink');
      mainLayout.text.classList.add('pink');
    }

    if(control === 3){
      mainLayout.text.classList.remove('pink');
      mainLayout.text.classList.add('purple');
    }

    mainLayout.text.innerHTML = messages[control];
    control++;
  }else{
    control = 1;
    mainLayout.text.innerHTML = 'Presiona el botón para ver la nota del mes C:';
  }
});